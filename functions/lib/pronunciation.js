...
exports.seedPronunciationSentences = exports.transcribeAndEvaluate = exports.getPronunciationSentences = void 0;
const admin = __importStar(require("firebase-admin"));
const https_1 = require("firebase-functions/v2/https");
const params_1 = require("firebase-functions/params");
const GOOGLE_STT_KEY = (0, params_1.defineSecret)('GOOGLE_STT_API_KEY');
const db = admin.firestore();
// ─── Helpers ───
/** Strip punctuation, normalize for comparison. */
const normalize = (w) => w.toLowerCase().replace(/[^a-zA-Z0-9']/g, '');
...
const evaluatePronunciation = (target, transcript) => {
    ...
    const score = { clarity, accuracy, fluency, overall };
 ...
 */
exports.getPronunciationSentences = (0, https_1.onCall)(async (request) => {
    const { difficulty, limit: reqLimit } = (request.data ?? {});
    const maxSentences = reqLimit ?? 3;
    let query = db.collection('pronunciation_sentences');
    if (difficulty) {
 ...
 */
exports.transcribeAndEvaluate = (0, https_1.onCall)({ secrets: [GOOGLE_STT_KEY], timeoutSeconds: 60, memory: '512MiB' }, async (request) => {
    const uid = request.auth?.uid;
    if (!uid) {
        throw new https_1.HttpsError('unauthenticated', 'You must be signed in.');
    }
    const { audioBase64, targetText, sentenceId, encoding, sampleRateHertz } = request.data;
    if (!audioBase64 || !targetText) {
        throw new https_1.HttpsError('invalid-argument', 'audioBase64 and targetText are required.');
    }
    ...
    try {
        ...
        throw new https_1.HttpsError('internal', 'Speech-to-text transcription failed. Please try again.');
    }
    if (!transcript) {
        throw new https_1.HttpsError('internal', 'Could not recognize any speech. Please speak more clearly and try again.');
    }
    // ── Evaluate pronunciation ──
    const { wordResults, score, feedback } = evaluatePronunciation(targetText, transcript);
 ...
 */
exports.seedPronunciationSentences = (0, https_1.onCall)(async (request) => {
    const uid = request.auth?.uid;
    if (!uid) {
