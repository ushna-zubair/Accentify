...
export const WavyChatOverlay: React.FC<{
  ...
  onExpand: () => void;
  expanded: boolean;
  lastResult?: import('../../controllers/useWavyChatController').WavyChatLastResult | null;
}> = ({ visible, lessonId, currentSentence, onClose, onExpand, expanded, lastResult }) => {
  const { colors: tc } = useAppTheme();
  const styles = useMemo(() => createStyles(tc), [tc]);
...
const PronunciationExerciseScreen: React.FC = () => {
  ...
  // Build the context Wavy needs to give an actually-useful tip about the
  // learner's most recent attempt (their score, which words failed, etc.).
  const wavyLastResult = useMemo(() => {
    if (!result) return null;
    return {
      reference: sentence?.text ?? '',
      overallPct: result.score.overall,
      weakWords: result.wordResults
  ...
  const handleComplete = async () => {
    ...
    if (!summary) return; // error surfaced via banner; stay on screen
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Segmented session progress — one bar per sentence */}
      ...
      >
        ...
        <View style={[styles.card, isResult && styles.cardResult]}>
          {/* Speaker button — plays the sentence with TTS */}
          ...
          {/* Processing spinner inside the card */}
          {isProcessing && (
